...
async function alertToSpeech(text, name) {
  const utterThat = new SpeechSynthesisUtterance(text);
  utterThat.voice = await getSpeechVoice(name);
  utterThat.volume = document.getElementById('alert-volume').value;
