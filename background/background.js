...
chrome.storage.local.get(`blockkList`, keys => {
  if (!keys.blockkList) {
    chrome.storage.local.set({ blockkList: {} });
  }
});

chrome.storage.local.get(`apincludeList`, keys => {
  if (!keys.apinclude) {
    chrome.storage.local.set({ apincludeList: {} });
  }
});
  ...
  if (!keys.hitFinder) {
    chrome.storage.local.set({
      hitFinder: {
        ...
        "display-filtered-column-time": true,
        "display-filtered-column-requester": true,
        "display-filtered-column-title": true,
        "display-filtered-column-available": true,
        "display-filtered-column-reward": true,
        "display-filtered-column-masters": true,
