...
chrome.storage.local.get([`hitFinder`, `blockList`, `includeList`, `blockkList`, `apincludeList`, `reviews`], (keys) => {
...
function pandaPush () {
  ...
  const oneFilter = pPushFunction();

  if (oneFilter) {
    alert(hit.requester_name);
  } 

  function pPushFunction() {
    return (hit.monetary_reward.amount_in_dollars) >= .10;
  }
}



function noQualFunction(hit) {
  return (hit.caller_meets_requirements) !== true; // inner function red
}

function qualFunction(hit) {
  return (hit.caller_meets_requirements === true);
}

function greenFunction(hit) {
return (hit.monetary_reward.amount_in_dollars >= 10.00); // global function
}

function notQualified (hit) {
  const [hit] = arguments

  if (hit.caller_meets_requirements !== true) {
    alert(hit.requester_id);
  }
...
function oneDollarFunction(hit) {
  return (hit.monetary_reward.amount_in_dollars) >= 1.00; // global function
}

function threeDollarFunction(hit) {
  return (hit.monetary_reward.amount_in_dollars) >= 3.00;
}

function fiveDollarFunction(hit) {
  return (hit.monetary_reward.amount_in_dollars) >= 5.00;
}

function tenDollarFunction(hit) {
  return (hit.monetary_reward.amount_in_dollars) >= 10.00;
}
...
function pandaLink(hit) { window.open(`https://worker.mturk.com/requesters/PandaCrazyAdd/projects?JRGID=${hit.hit_set_id}&JRRName=${hit.requester_name}&JRRID=${hit.requester_id}&JRTitle=${hit.title}&JRReward=${toMoneyString(hit.monetary_reward.amount_in_dollars)}&secondsOffEdit:600`); }


// ----------------------------------------------------------------------------------- Finder Process Function ----------------------------------------------------------------------------
...
function finderProcess () {
  return new Promise(async (resolve) => {
    ...
    for (const hit of json.results) {
      ...
      const filtered = tenDollarFunction(hit);
      const qualified = qualFunction(hit)
      const notQualified = noQualFunction(hit) // This is what decides what is qualified.


// ------------------------------------------------------------------------------------------------- AP Filters --------------------------------------------------------------------------------

      const oneDollarFilter = oneDollarFunction(hit)
      const threeDollarFilter = threeDollarFunction(hit)
      const fiveDollarFilter = fiveDollarFunction(hit)
      const tenDollarFilter = tenDollarFunction(hit)
...
// ------------------------------------------------------------------------------------------------ Change Class -------------------------------------------------------------------------------
      ...
      if (included) {
        row.classList.add(`included`) // green border
      }
      ...
      if (notQualified) {
        row.classList.add(`no-qual`) // red
        //audioCensor.play();
      }
      
      if (oneDollarFilter) {
        row.classList.add(`green-dollar`) // yellow
        //audioCensor.play();
      }

      if (tenDollarFilter) {
        row.classList.add(`ten-dollar-border`) // orange
      }
      
      if (qualified && threeDollarFilter) {
        row.classList.add(`three-dollar-border`)
      }

      if (qualified && tenDollarFilter) {
        row.classList.add(`ten-dollar-border-plus`) // orange
      }
...
// ----------------------------------------------------------------------------------------- Construct the Recent Hits Row ----------------------------------------------------------------------
      ...
      const filteredRow = toggleColumns(row.cloneNode(true), `filtered`)
      filteredRow.id = `filtered-${hit.hit_set_id}`
    ...
    // ------------------------------------------------------------------ This is where the results for filtered are printed out. ---------------------------------------------------------------

    const filteredElement = document.getElementById(`filtered-${hit.hit_set_id}`)
    if (filteredElement) filteredElement.replaceWith(filteredRow) // This is what adds the filtered hit.

    else

            if (filtered && qualified) { // ------------------------------------------ This is where we set our filters. ------------------------------------------------------------------------
              document.getElementById(`filtered-hits-card`).style.display = ``
              row.classList.add(`green-dollar-border`); // yellow
              pandaLink(hit);
              filteredFragment.appendChild(filteredRow)
          
            }else{
                if (threeDollarFilter) { // ------------------------------------- This is where we print out what is not fired to PC. ------------------------------------------------------------
        document.getElementById(`filtered-hits-card`).style.display = ``
        //row.classList.add(`green-dollar-border`); // yellow
        //row.classList.add(`no-qual`); // red
        filteredFragment.appendChild(filteredRow)
    ...
    toggleColumns(document.getElementById(`filtered-hits-thead`).children[0], `filtered`) // Chops off a section of the row if commented out.
    ...
    document.getElementById(`filtered-hits-tbody`).insertBefore(filteredFragment, document.getElementById(`filtered-hits-tbody`).firstChild) // Chops off a row if commented out.
    ...
    //document.getElementById(`hits-found`).textContent = `Found: ${json.num_results} | Blockked: ${blockked} | ${new Date().toLocaleTimeString()}`
    document.getElementById(`hits-logged`).textContent = document.getElementById(`logged-hits-tbody`).children.length
    document.getElementById(`hits-filtered`).textContent = document.getElementById(`filtered-hits-tbody`).children.length // Hits filtered counter
...
function pandaPull (hit) {
  //const [hit] = arguments
    if (hit.monetary_reward.amount_in_dollars >= .10) {
      alert(hit.requester_name);
    }
  }
...
function blockkListed (hit) {
  for (const match in storage.blockkList) {
    const bl = storage.blockkList[match]
    if (bl.strict) {
      const compared = strictCompare(match, [hit.hit_set_id, hit.requester_id, hit.requester_name, hit.title])
      if (compared === true) {
        return true
...
function apincludeListed (hit) {
  for (const match in storage.apincludeList) {
    const al = storage.apincludeList[match]
    if (al.strict) {
      const compared = strictCompare(match, [hit.hit_set_id, hit.requester_id, hit.requester_name, hit.title])
  if (compared === false) {
    return al
...
function blockListUpdate () {
  ...
  for (const key in finderDB) {
    ...
    if (blockListed(hit)) {
     ...
     // const filtered = document.getElementById(`filtered-${hit.hit_set_id}`)
      const included = document.getElementById(`included-${hit.hit_set_id}`)
      const apincluded = document.getElementById(`apincluded-${hit.hit_set_id}`)
      ...
      if (apincluded) apincluded.parentNode.removeChild(apincluded)
...
function blockkListUpdate () {
  const sorted = Object.keys(storage.blockkList).map((currentValue) => {
    storage.blockkList[currentValue].term = currentValue
    return storage.blockkList[currentValue]
  }).sort((a, b) => a.name.localeCompare(b.name, `en`, { numeric: true }))

  const body = document.getElementById(`blockk-list-modal`).getElementsByClassName(`modal-body`)[0]
  ...
  body.appendChild((() => {
    ...
    for (const bbl of sorted) {
      const button = document.createElement(`button`)
      button.type = `button`
      button.className = `btn btn-sm btn-danger ml-1 my-1 bl-btn`
      button.textContent = bbl.name
      button.dataset.toggle = `modal`
      button.dataset.target = `#blockk-list-edit-modal`
      button.dataset.key = bl.match
      fragment.appendChild(button)
    }
  ...
  for (const key in finderDB) {
    ...
    if (blockkListed(hit)) {
      const recent = document.getElementById(`recent-${hit.hit_set_id}`)
      const logged = document.getElementById(`logged-${hit.hit_set_id}`)
     // const filter = document.getElementById(`filtered-${hit.hit_set_id}`)
      const included = document.getElementById(`included-${hit.hit_set_id}`)
      const apincluded = document.getElementById(`apincluded-${hit.hit_set_id}`)
      ...
      if (logged) logged.parentNode.removeChild(logged)
      if (included) included.parentNode.removeChild(included)
    //  if (filter) apincluded.parentNode.removeChild(apincluded)
      if (apincluded) apincluded.parentNode.removeChild(apincluded)
...
function apincludeListUpdate () {
  const sorted = Object.keys(storage.apincludeList).map((currentValue) => {
    storage.apincludeList[currentValue].match = currentValue
    return storage.apincludeList[currentValue]
  }).sort((a, b) => a.name.localeCompare(b.name, `en`, { numeric: true }))

  const body = document.getElementById(`apinclude-list-modal`).getElementsByClassName(`modal-body`)[0]
  ...
  body.appendChild((() => {
    ...
    for (const al of sorted) {
      ...
      button.dataset.toggle = `modal`
      button.dataset.target = `#apinclude-list-edit-modal`
...
/*
    ...
    if (element) {
      if (apincludeListed(hit)) {
        element.classList.add(`apincluded`)
      } else {
        element.classList.remove(`apincluded`)
      }
    }*/
  }

  chrome.storage.local.set({
    apincludeList: storage.apincludeList
  })
}
...
// -----------------------------------------------------------------------------  Start functions here (maybe)  -------------------------------------------------------------------------------

function pandaPush (hit) {
  const [hit] = arguments

  const oneDollarFilter = (hit.monetary_reward.amount_in_dollars) >= 1.00;
...
$(`#blockk-list-add-modal`).on(`show.bs.modal`, (event) => {
  const name = event.relatedTarget.dataset.name
  const match = event.relatedTarget.dataset.match

  document.getElementById(`blockk-list-add-name`).value = name || ``
  document.getElementById(`blockk-list-add-match`).value = match || ``
  document.getElementById(`blockk-list-add-strict`).checked = true
})

$(`#blockk-list-edit-modal`).on(`show.bs.modal`, (event) => {
  const key = event.relatedTarget.dataset.key
  const item = storage.blockkList[key]

  document.getElementById(`blockk-list-edit-name`).value = item.name
  document.getElementById(`blockk-list-edit-match`).value = item.match
  document.getElementById(`blockk-list-edit-strict`).checked = item.strict

  document.getElementById(`blockk-list-edit-delete`).dataset.key = key
})
...
// ------------------------------------------------------------------------------------------ AP Include List Modal -------------------------------------------------------------------------------

$(`#apinclude-list-add-modal`).on(`show.bs.modal`, (event) => {
  const name = event.relatedTarget.dataset.name
  const match = event.relatedTarget.dataset.match

  document.getElementById(`apinclude-list-add-name`).value = name || ``
  document.getElementById(`apinclude-list-add-match`).value = match || ``
  document.getElementById(`apinclude-list-add-strict`).checked = true
})

$(`#apinclude-list-edit-modal`).on(`show.bs.modal`, (event) => {
  const key = event.relatedTarget.dataset.key
  const item = storage.apincludeList[key]

  document.getElementById(`apinclude-list-edit-name`).value = item.name
  document.getElementById(`apinclude-list-edit-match`).value = item.match
  document.getElementById(`apinclude-list-edit-strict`).checked = item.strict

  document.getElementById(`apinclude-list-edit-delete`).dataset.key = key
})
...
$(`#hit-info-modal`).on(`show.bs.modal`, (event) => {
  ...
  document.getElementById(`hit-info-blockk-requester`).dataset.key = key
  document.getElementById(`hit-info-blockk-requester`).dataset.name = hit.requester_name
  document.getElementById(`hit-info-blockk-requester`).dataset.match = hit.requester_id

  document.getElementById(`hit-info-blockk-hit`).dataset.key = key
  document.getElementById(`hit-info-blockk-hit`).dataset.name = hit.title
  document.getElementById(`hit-info-blockk-hit`).dataset.match = hit.hit_set_id
  ...
  document.getElementById(`hit-info-apinclude-requester`).dataset.key = key
  document.getElementById(`hit-info-apinclude-requester`).dataset.name = hit.requester_name
  document.getElementById(`hit-info-apinclude-requester`).dataset.match = hit.requester_id

  document.getElementById(`hit-info-apinclude-hit`).dataset.key = key
  document.getElementById(`hit-info-apinclude-hit`).dataset.name = hit.title
  document.getElementById(`hit-info-apinclude-hit`).dataset.match = hit.hit_set_id
...
// document.getElementById(`logged`).addEventListener(`change`, pullPanda)
...
document.getElementById(`block-list-edit-delete`).addEventListener(`click`, (event) => {
  ...
  delete storage.blockkList[key]
  blockkListUpdate()
})

// ------------------------------------------------------------------------------------------ Blockk List Options ---------------------------------------------------------------------------------------------

document.getElementById(`blockk-list-delete`).addEventListener(`click`, (event) => {
  const result = window.confirm(`Are you sure you delete your entire Blockk List?`)
...
document.getElementById(`blockk-list-export`).addEventListener(`click`, (event) => {
  saveToFileJSON(`blockk-list`, storage.blockList)
})

document.getElementById(`blockk-list-import`).addEventListener(`change`, async (event) => {
  const json = await loadFromFileJSON(event.target.files[0])
...
document.getElementById(`blockk-list-add-save`).addEventListener(`click`, (event) => {
  const name = document.getElementById(`blockk-list-add-name`).value
  const match = document.getElementById(`blockk-list-add-match`).value

  if (name.length && match.length) {
    storage.blockList[match] = {
      name: name,
      match: match,
      strict: document.getElementById(`blockk-list-add-strict`).checked
    }
...
document.getElementById(`blockk-list-edit-save`).addEventListener(`click`, (event) => {
  const name = document.getElementById(`blockk-list-edit-name`).value
  const match = document.getElementById(`blockk-list-edit-match`).value

  if (name.length && match.length) {
    storage.blockList[match] = {
      name: name,
      match: match,
      strict: document.getElementById(`blockk-list-edit-strict`).checked
    }
...
document.getElementById(`blockk-list-edit-delete`).addEventListener(`click`, (event) => {
  const key = event.target.dataset.key
  delete storage.blockkList[key]
  blockkListUpdate()
})
...
// ------------------------------------------------------------------------------ APInclude Settings Modal ---------------------------------------------------------------------------------------------

// start autoPanda thingy here

document.getElementById(`apinclude-list-delete`).addEventListener(`click`, (event) => {
  const result = window.confirm(`Are you sure you delete your entire autoPanda List?`)

  if (result) {
    storage.apincludeList = {}
    apincludeListUpdate()
  }
})

document.getElementById(`apinclude-list-import`).addEventListener(`change`, async (event) => {
  const json = await loadFromFileJSON(event.target.files[0])
...
document.getElementById(`apinclude-list-export`).addEventListener(`click`, (event) => {
  saveToFileJSON(`apinclude-list`, storage.apincludeList)
})

document.getElementById(`apinclude-list-add-save`).addEventListener(`click`, (event) => {
  const name = document.getElementById(`apinclude-list-add-name`).value
  const match = document.getElementById(`apinclude-list-add-match`).value

  if (name.length && match.length) {
    storage.apincludeList[match] = {
      name: name,
      match: match,
      strict: document.getElementById(`apinclude-list-add-strict`).checked,
      sound: document.getElementById(`apinclude-list-add-sound`).checked,
      alarm: document.getElementById(`apinclude-list-add-alarm`).checked,
    }
...
document.getElementById(`apinclude-list-edit-save`).addEventListener(`click`, (event) => {
  const name = document.getElementById(`apinclude-list-edit-name`).value
  const match = document.getElementById(`apinclude-list-edit-match`).value

  if (name.length && match.length) {
    storage.apincludeList[match] = {
      name: name,
      match: match,
      strict: document.getElementById(`apinclude-list-edit-strict`).checked,
      sound: document.getElementById(`apinclude-list-edit-sound`).checked,
      alarm: document.getElementById(`apinclude-list-edit-alarm`).checked,
      notification: document.getElementById(`apinclude-list-edit-notification`).checked,
      pushbullet: document.getElementById(`apinclude-list-edit-pushbullet`).checked
    }
...
document.getElementById(`apinclude-list-edit-delete`).addEventListener(`click`, (event) => {
  const key = event.target.dataset.key
  delete storage.apincludeList[key]
  apincludeListUpdate()
})
...
document.getElementById(`include-list-hits-toggle`).addEventListener(`click`, (event) => {
  const classList = document.getElementById(`include-list-hits-toggle`).firstElementChild.classList
  classList.toggle(`fa-caret-up`)
  classList.toggle(`fa-caret-down`)

  const element = document.getElementById(`include-list-hits-card`).getElementsByClassName(`card-block`)[0]
  element.style.display = element.style.display === `none` ? `` : `none`
})

document.getElementById(`include-list-hits-clear`).addEventListener(`click`, (event) => {
...
document.getElementById(`filtered-hits-toggle`).addEventListener(`click`, (event) => {
  const classList = document.getElementById(`filtered-hits-toggle`).firstElementChild.classList
  classList.toggle(`fa-caret-up`)
  classList.toggle(`fa-caret-down`)

  const element = document.getElementById(`filtered-hits-card`).getElementsByClassName(`card-block`)[0]
  element.style.display = element.style.display === `none` ? `` : `none`
})

document.getElementById(`filtered-hits-clear`).addEventListener(`click`, (event) => {
  document.getElementById(`filtered-hits-card`).style.display = `none`
  removeChildren(document.getElementById(`filtered-hits-tbody`))
})
...
function alertFunction() {
  alertToSpeech('Alert level test');
}
